...
- Storage is one `chrome.storage.local` key, `mrHighlights`, a map of canonical URL to an array of items. An earlier version wrapped that array in `{ savedAt, items }`, so both read points accept either and the next write rewrites the key flat. Dropping that coercion strands every page marked before the change — not as empty data but as a hard failure, since the object reaches `push`/`filter`. One key means the side panel can watch a single `storage.onChanged` entry. Nothing prunes it: an item is a few hundred bytes against a 10MB quota, so an eviction policy would cost more code than it saves.
...
- Nothing is focused for a fresh selection. Focusing the field collapses the document selection, which silently breaks Ctrl+C and the native right-click Copy on the text the user just selected. Instead a document `keydown` watches while the popup is open: a bare printable key focuses the field and seeds it with that character, so typing still needs no click, while Ctrl/Cmd/Alt combinations fall through to the browser untouched. The field is focused outright only when an existing mark is clicked, where there is no selection to lose.
- The popup is left-button only (`event.button !== 0` returns) and `contextmenu` hides it, so a right-click goes straight to the browser's menu with the selection intact.
- A selection inside an `input`, `textarea` or `contenteditable` never opens the popup. Highlighting a form field is meaningless, and the type-to-focus handler would otherwise swallow what the user types into the page's own fields.
